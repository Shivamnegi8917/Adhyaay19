...
export const loadUser = () => (dispatch, getState) => {
  ...
  axios
    .get("https://gentle-dusk-33875.herokuapp.com/api/auth/user", tokenConfig(getState))
    .then(res =>
      dispatch({
        type: USER_LOADED,
        payload: res.data
    ...
    .catch(err => {
      if(err.response && err.response.data) dispatch(returnErrors(err.response.data, err.response.status));
      dispatch({
        type: AUTH_ERROR
      });
    });
